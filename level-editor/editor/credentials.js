...
function updateUserInterface(user) {
  ...
  const signInState = document.querySelector('.state.sign_in');
  const topAvatar = document.querySelector('#topAvatar');
  
  if (signedInState && signInState) {
    if (user) {
      signedInState.style.display = 'block';
      signInState.style.display = 'none';
      
      // Update user info
      const nameElement = signedInState.querySelector('#name');
      const emailElement = signedInState.querySelector('#email');
      const avatarElement = signedInState.querySelector('#avatar');
      
      if (nameElement) nameElement.textContent = user.displayName || 'User';
      if (emailElement) emailElement.textContent = user.email;
      if (avatarElement) {
        avatarElement.src = user.photoURL || 'avatar-generic.png';
      }
      
      // Update top avatar
      if (topAvatar) {
        topAvatar.src = user.photoURL || 'avatar-generic.png';
      }
    } else {
      ...
      // Reset top avatar
      if (topAvatar) {
        topAvatar.src = 'avatar-generic.png';
      }
    }
  }
}
...
async function loadUserData(user) {
  try {
    ...
    if (doc.exists) {
      ...
      const userAvtElement = document.querySelector('.user_avt');
      ...
      if (userAvtElement && userData.photoURL) {
        userAvtElement.src = userData.photoURL;
      }
    }
  } catch (error) {
