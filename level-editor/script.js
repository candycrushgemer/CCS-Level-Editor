...
const gumball = {"229": "gumball_machine_empty", "230": "gumball_machine"}
const candycobra = {"222": "cobra_5", "223": "cobra_4", "224": "cobra_3", "225": "cobra_2", "226": "cobra_1", "227": "cobra_basket"}
...
const elements_ids = Object.assign({}, rainbowcannontop, rainbowcannonbottom, rainbowcannonleft, rainbowcannonright, leaflayer, colors, cannons, walldown, wallup, bonbon, path, wallright, wallleft, coloredCandy, candy, blockers, tiles, ingredients, sugarCoats, locks, glass, gumball, candycobra, portalentrance, portalexit, {"010": "ingredients_exit", "026": "candy_entrance", "005": "candy_cannon"})
...
const orderItems = {"1": "red", "2": "blue", "3": "yellow", "4": "orange", "5": "purple", "6": "green", "7": "wrapped", "8": "striped", "9": "colorbomb", "10": "striped + striped", "11": "striped + wrapped", "12": "striped + colorbomb", "13": "colorbomb + colorbomb", "14": "wrapped + colorbomb", "15": "wrapped + wrapped", "16": "chocolate", "17": "frosting", "18": "licorice shell", "19": "licorice", "20": "pepper bomb", "21": "jellyfish", "22": "cake bomb", "23": "mystery candy", "24": "magic mixer", "25": "waffle", "26": "dark chocolate", "27": "candy cane curl", "28": "crystal candy", "29": "rainbow twist", "30": "frog", "31": "sugar coat", "32": "bubblegum", "33": "licorice curl", "34": "sour skull", "35": "bonbon blitz", "36": "jelly jar", "37": "candy cobra", "38": "wonderful wrapper", "39": "gumballs"}
...
const layerElements = {
    ...
    "normal": [].concat(Object.keys(colors), Object.keys(coloredCandy), Object.keys(candy), Object.keys(blockers), Object.keys(ingredients), Object.keys(bonbon), Object.keys(gumball), Object.keys(candycobra)),
...
function addRequirement(isIngredient = false, ignoreLimit = false){
    ...
    section.innerHTML = '<button style="position: relative; border-radius: 50px; border: none; background-color: #ffffff00; width: 40px; height: 40px; display: flex; justify-content: center; align-items: center;" onclick="removeRequirement(this)"><md-icon>delete</md-icon><md-focus-ring></md-focus-ring><md-ripple></md-ripple></button> <br> <br> <img src="ui/hud/red.png" style="max-width: 30px; max-height: 30px; padding-right: 5px;"><select onchange="switchedRequirement(this)"> </select> <div class="break"></div> <p  style="margin: 10px; display: block; text-align: center;">Amount:</p> <div class="wave-group"><input type="number" class="input" placeholder="0" style="width: 100px;"><span class="bar"></span><span class="label-char" style="--index: 0"></span></div>'
...
function importLevel(levelData){
    ...
    // Read candyCobras and create properties
    if (levelData.candyCobras) {
        levelData.candyCobras.forEach(function(cobra) {
            let listItem = document.createElement('md-list-item');
            listItem.innerHTML = `
                <md-list-item>
                    <button class="icon-button" id="delCobraPp" slot="start">
                        <span class="sf-icon"></span>
                    </button>
                    Candy Cobra [${cobra.join(', ')}]
                </md-list-item>
            `;
            document.getElementById('cobraproperty').appendChild(listItem);
        });
    }
}

// Initialize the export field by removing default values
function initializeExportField() {
    const exportField = document.getElementById('exportfield');
    exportField.value = exportField.value
        .replace(/"candyCobras":\[\[null,null,null\]\],?/g, '') // Remove the null default
        .replace(/"candyCobras":3,?/g, ''); // Remove the candyCobras:3 default
}

// Call the initialization function on page load
initializeExportField();
...
document.getElementById('addProperty').addEventListener('click', function() {
    ...
    const startTileCobra = document.getElementById('startTileCobra').value;
    const cobraLayer = document.getElementById('cobraLayer').value;
    const endTileCobra = document.getElementById('endTileCobra').value;
    const exportField = document.getElementById('exportfield');

    // Create a new md-list-item
    const listItem = document.createElement('md-list-item');
    listItem.innerHTML = `
        <button class="icon-button" slot="start">
            <span class="sf-icon"></span>
        </button>
        Candy Cobra <span class="cobraNumberic">${document.querySelectorAll('md-list-item').length + 1}</span>
    `;

    // Append the new list item to the appropriate container
    document.getElementById('cobraproperty').appendChild(listItem);
    ...
    // Update the export field when a new property is added
    updateExportField();

    // Function to update the export field based on the current list items
    function updateExportField() {
        const items = document.querySelectorAll('#cobraproperty md-list-item');
        const properties = Array.from(items).map(() => {
            return `[${startTileCobra},${cobraLayer},${endTileCobra}]`; // Use the actual values
        });

        // Remove the default line if it exists
        exportField.value = exportField.value.replace(/"candyCobras":\[\[null,null,null\]\],?/g, '');

        // If there are properties, set the new value for candyCobras
        if (properties.length > 0) {
            exportField.value = `"candyCobras":[${properties.join(',')}]`;
        } else {
            exportField.value = ''; // Reset to empty if no items
        }
    }

    // Check if exportField has a value
    if (exportField.value) {
        // Remove the default line if it exists
        exportField.value = exportField.value.replace(/"candyCobras":\[\[null,null,null\]\],?/g, '');

        // Create the new data property string
        const newDataProperty = `[${startTileCobra},${cobraLayer},${endTileCobra}]`;

        // If the exportField is empty or doesn't contain the candyCobras line, initialize it
        if (!exportField.value.includes('"candyCobras":')) {
            exportField.value = `"candyCobras":[[${startTileCobra},${cobraLayer},${endTileCobra}]]`;
        } else {
            // If it already contains candyCobras, we need to replace the existing entry
            const regex = /"candyCobras":\[\[.*?\]\]/; // Match the existing candyCobras array
            exportField.value = exportField.value.replace(regex, `"candyCobras":[[${startTileCobra},${cobraLayer},${endTileCobra}]]`); // Replace with new property
        }
    } else {
        // If exportField is empty, initialize it with the first property
        exportField.value = `"candyCobras":[[${startTileCobra},${cobraLayer},${endTileCobra}]]`;
    }
});
...
const quickOpenBtns = document.querySelectorAll('.openlvl');
...
// Add click event listeners to all quick open buttons
quickOpenBtns.forEach(btn => {
...
function exportLevel(){
    ...
    level['portals'] = []  // Initialize portals array
    level['gates'] = []
    level['orlocks'] = []
    level['skulls'] = []
    ...
    level["candyCobras"] = parseInt(document.getElementById('evilSpawnerInterval').value) || 3
    ...
    return level
    deductPoint(); // Deduct point when exporting
}
...
document.querySelectorAll(".cannonpref").forEach(function(element){
    ...
    element.innerHTML = '<td> <img class="elmimg" style="max-width: 70%;"> </td> <td> <div class="wave-group"><input type="number" class="input max" placeholder="0" style="width: 50px;"><span class="bar"></span><span class="label-char" style="--index: 0"></span></div> </td> <td> <div class="wave-group"><input type="number" class="input spawn" placeholder="0" style="width: 50px;"><span class="bar"></span><span class="label-char" style="--index: 0"></span></div> </td> <td> <div class="wave-group"><input type="number" class="input tick" placeholder="0" style="width: 50px;"><span class="bar"></span><span class="label-char" style="--index: 0"></span></div> </td>'
...
class MatCircularProgress extends HTMLElement {
    ...
    render() {
      ...
      iframe.src = 'mat-circular-progress.html';
...
const urlMap = new Map(); // In-memory storage for URL mappings



const slider = document.querySelector(".slider");
const valuedTrack = document.querySelector(".valued-track");
...
function updateSlider() {
  ...
  const thumbWidth = 14; // Updated thumb width
  const spacing = 5; // Spacing from the thumb

  // Calculate the percentage value of the slider
  const percentage = sliderValue / slider.max;

  // Calculate thumb position in pixels
  const thumbPosition = percentage * sliderWidth;

  // Set the valued track width
  valuedTrack.style.width = `${thumbPosition - spacing}px`;

  // Set the background track width
  backgroundTrack.style.width = `${sliderWidth - thumbPosition - spacing}px`;

  // Move the custom thumb
  thumb.style.left = `${thumbPosition - thumbWidth / 2}px`;

  // Update the popover position and value
  popover.textContent = sliderValue; // Set the value
  popover.style.left = `calc(${percentage * 100}% - 50%)`; // Center the popover dynamically
}

// Show popover when slider is interacted with
slider.addEventListener("mousedown", () => {
  popover.classList.add("show");
});

// Hide popover on mouse up or when interaction stops
slider.addEventListener("mouseup", () => {
  setTimeout(() => {
    popover.classList.remove("show");
  }, 200); // Add delay for fade-out animation
});

// Update slider and popover value dynamically
...
class M3Slider extends HTMLElement {
  constructor() {
    ...
    shadow.innerHTML = `
      <style>
        ...
        .popover {
          ...
          font-size: 12px;
          border-radius: 6px;
          box-shadow: 0px 2px 4px rgba(var(--md-sys-color-background));
          z-index: 4;
          white-space: nowrap;
          opacity: 0;
          visibility: hidden;
    ...
    const valuedTrack = shadow.querySelector(".valued-track");
    const backgroundTrack = shadow.querySelector(".background-track");
    const thumb = shadow.querySelector(".thumb");
    const popover = shadow.querySelector(".popover");

    // Function to update slider components
    const updateSlider = () => {
      const sliderWidth = slider.offsetWidth;
      const sliderValue = slider.value;
      const thumbWidth = 14;
      ...
      // Update the popover position and value
      popover.textContent = sliderValue;
      popover.style.left = `calc(${percentage * 100}% - 50%)`;
    };

    // Show popover when interacting with the slider
    slider.addEventListener("mousedown", () => {
      popover.classList.add("show");
    });

    // Hide popover after interaction ends
    slider.addEventListener("mouseup", () => {
      setTimeout(() => {
        popover.classList.remove("show");
      }, 200);
...
window.addEventListener('scroll', () => {
    ...
    if (window.innerHeight + window.scrollY >= document.body.offsetHeight) {
        ...
        scrollableContent.classList.remove('end-anim'); // Remove shake class if present
    }
});

// Detect when the user scrolls beyond the bottom of the page
window.addEventListener('wheel', (event) => {
    if (window.innerHeight + window.scrollY >= document.body.offsetHeight) {
        // User is trying to scroll down but is at the bottom
        scrollableContent.classList.add('end-anim'); // Add shake animation class

        // Remove the shake class after the animation ends
        setTimeout(() => {
            scrollableContent.classList.remove('end-anim');
